...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.power(self.nodes[0, :], 1) + np.power(self.nodes[1, :], 0)

        # Тут может быть скорость, но сейчас здесь нули
        #self.velocity = np.zeros(shape=(3, 8776) , dtype=np.double)
        self.velocity = np.zeros(shape=(3, 348556) , dtype=np.double)
        #self.velocity[0] = np.power(np.abs(self.nodes[0] + self.nodes[1]), 0.5)
        self.velocity[0] = np.power(self.nodes[0], 2)
        self.velocity[1] = np.power(self.nodes[1], 2)
        self.velocity[2] = np.power(self.nodes[2], 2)
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("b/tetr3d-step-" + str(snap_number) + ".vtu")
        writer.Write()
